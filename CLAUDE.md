...
Keep this repository free of assistant attribution. The rule covers commits,
pull requests, code comments, and every other file you push.

### Rules

1. Set the author and the committer to
   `retr0astic <sreejiraj2399@gmail.com>`.
2. Do not add a `Co-Authored-By` trailer.
3. Do not add a `Claude-Session` trailer or a session link.
4. Do not add a "Generated by" footer to a pull request body or a comment.
5. Do not name a model or a vendor in any text you push.

### Check

Run this before you push. It finds attribution lines and the wrong author.

    git log --format='%an <%ae>%n%B' origin/testing..HEAD |
      grep -nE '^(Co-[Aa]uthored-[Bb]y|Claude-Session):|anthropic'
